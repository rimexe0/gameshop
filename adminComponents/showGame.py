...
def userpage_game(win, vrow, vcol, username):
    ...
    try:
        ...
        game_name = CTkLabel(game_text, text=game[0]['name'])
        game_name.configure(font=('Arial', 30))
        game_last_played = CTkLabel(game_text, text=("last played " + str(game[0]['last_played'])))
        game_hours_played = CTkLabel(game_text, text=str(game[0]['hours_played']) + " hours played")
        image = Image.showImage(show_game_frame, game[0]['image'], 0, 0, 150, 150)
