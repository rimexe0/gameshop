...
def game_achievements(win, game_id, user_id):
    achievement_list = CTkFrame(win, width=200, height=50, fg_color="#2b2b2b")
...
def userpage_game(win, vrow, vcol, game, user):
    ...
    game_frame.grid(row=vrow, column=vcol, padx=5, pady=5)
...
def userpage_game_list(win, vrow, vcol, user):
    ...
    try:
        ...
        for i in game:
            print(i)
            userpage_game(gamelist_frame, i['id'], 1, i, user)
